...
// *** ИСПРАВЛЕННАЯ ФУНКЦИЯ ГРУППОВОГО УДАЛЕНИЯ ***
const handleDeleteSelected = useCallback((): void => {
  ...
  // *** СОЗДАЕМ ОДИН ДИАЛОГ ДЛЯ ВСЕХ ВЫБРАННЫХ ЗАПИСЕЙ ***
  const selectedIds = Array.from(selectedRows);
  console.log(`[ScheduleTable] Setting up bulk delete for ${selectedIds.length} items:`, selectedIds);
  ...
  setConfirmDialogProps({
    ...
    onConfirm: async () => {
      ...
      // *** УДАЛЯЕМ ВСЕ ЗАПИСИ ПОСЛЕДОВАТЕЛЬНО ***
      let successCount = 0;
      ...
      // *** ЛОГИРУЕМ РЕЗУЛЬТАТ ***
      console.log(`[ScheduleTable] Bulk deletion completed: ${successCount}/${selectedIds.length} successful, ${failedIds.length} failed`);
      ...
      // *** ПОКАЗЫВАЕМ РЕЗУЛЬТАТ ПОЛЬЗОВАТЕЛЮ (опционально) ***
      if (failedIds.length === 0) {
        console.log(`[ScheduleTable] All ${successCount} items deleted successfully`);
      } else {
        console.warn(`[ScheduleTable] ${successCount} items deleted, ${failedIds.length} failed`);
      }
    },
    confirmButtonColor: '#d83b01'
  });
}, [selectedRows, onDeleteItem]);
      ...
      <div className="pagination-container" style={{
        ...
        <div className="navigation-buttons" style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <DefaultButton
            ...
            disabled={!calculatedHasPreviousPage || isLoading} // ← ИСПРАВЛЕНО
            styles={{ 
              root: { 
                minWidth: '80px',
                backgroundColor: !calculatedHasPreviousPage ? '#f5f5f5' : '#fff' // ← ИСПРАВЛЕНО
              }
            }}
          />
          ...
          <DefaultButton
            ...
            disabled={!calculatedHasNextPage || isLoading} // ← ИСПРАВЛЕНО
            styles={{ 
              root: { 
                minWidth: '80px',
                backgroundColor: !calculatedHasNextPage ? '#f5f5f5' : '#fff' // ← ИСПРАВЛЕНО
              }
            }}
          />
        </div>
