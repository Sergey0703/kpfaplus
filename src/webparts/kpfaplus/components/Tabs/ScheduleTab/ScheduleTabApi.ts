...
const activeLeavesData = leavesData.filter(leave => {
  ...
  if (isDeleted) {
    console.log(`[ScheduleTab] Filtering out deleted leave: ${leave.title} (${new Date(leave.startDate).toLocaleDateString()} - ${leave.endDate ? new Date(leave.endDate).toLocaleDateString() : 'ongoing'})`);
  }
  return !isDeleted;
});
  
  console.log(`[ScheduleTab] Retrieved ${leavesData.length} total leaves, ${activeLeavesData.length} active leaves for month ${date.getMonth() + 1} and year ${date.getFullYear()}`);
  
  if (setLeaves) {
    setLeaves(activeLeavesData); // Use filtered data
  }
  
  // Логируем первые несколько активных отпусков для проверки
  if (activeLeavesData.length > 0) {
    const sampleLeaves = activeLeavesData.slice(0, 3);
    console.log("[ScheduleTab] Sample active leaves:", sampleLeaves);
  }
  
  return activeLeavesData; // Return filtered data
} catch (err) {
