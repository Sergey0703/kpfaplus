...
export const useStaffRecordsMutations = (
 ...
 // *** ОБНОВЛЕННАЯ ФУНКЦИЯ handleAddShift ТОЛЬКО С ЧИСЛОВЫМИ ПОЛЯМИ ***
 const handleAddShift = useCallback(async (date: Date, shiftData?: INewShiftData): Promise<void> => {
     console.log(`[useStaffRecordsMutations] *** HANDLE ADD SHIFT WITH NUMERIC FIELDS ONLY ***`);
     ...
     console.log('[useStaffRecordsMutations] *** CREATING NEW SHIFT WITH NUMERIC FIELDS ONLY ***');
     console.log('[useStaffRecordsMutations] Numeric time fields:', {
     ...
     await handleMutation(
         ...
         'Shift added successfully with numeric time fields only.',
         'add shift'
     );
